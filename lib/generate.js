...
2. Les appels d'animation GSAP à ajouter sur le timeline "tl" déjà déclaré (pas sa déclaration)
...
- Chaque élément avec class="clip" DOIT aussi avoir un id unique et lisible (ex: id="titre-principal", id="photo-medaillon") — obligatoire, pas optionnel
- La variable "tl" existe DÉJÀ (déclarée par le scaffold : const tl = gsap.timeline({ paused: true });) — NE LA REDÉCLARE JAMAIS. Ton timelineScript ne doit contenir QUE des appels comme tl.from(...), tl.to(...), jamais "const tl = ..." ni "let tl = ..."
- Le timeline est déjà enregistré par le scaffold (window.__timelines["main"] = tl;) — ne le fais pas toi-même non plus
- Sur une même data-track-index, les clips ne doivent JAMAIS se chevaucher dans le temps (start à start+duration) — si deux éléments sont visibles en même temps, donne-leur des data-track-index différents
- N'utilise QUE des polices web-safe (Arial, Georgia, Times New Roman, Helvetica, Verdana, sans-serif, serif) ou celles déjà chargées par le scaffold (Inter, JetBrains Mono) — jamais de police décorative (ex: Brush Script MT, Comic Sans, Pacifico) sans déclaration @font-face, ce qui n'est pas supporté ici
- Aucune logique non déterministe : pas de Date.now(), Math.random(), ni d'appel réseau
- Si une photo est fournie, utilise le placeholder <img src="{{PHOTO_SRC}}"> dans une balise <img class="clip" id="..." ...> — ne l'omets pas, ne l'invente pas

Réponds EXACTEMENT dans ce format, sans rien ajouter avant ni après (pas de JSON, pas de markdown, pas d'explication) :
===ROOT===
(le contenu HTML à placer dans #root, tel quel, sans échappement)
===TIMELINE===
(le script GSAP, tel quel, sans échappement)
===END===`;
...
async function callClaude(systemPrompt, userPrompt) {
  ...
  const rootMatch = text.match(/===ROOT===([\s\S]*?)===TIMELINE===/);
  const timelineMatch = text.match(/===TIMELINE===([\s\S]*?)===END===/);

  if (!rootMatch || !timelineMatch) {
    throw new Error(
      `Réponse Claude mal formée (délimiteurs introuvables). Début de la réponse : ${text.slice(0, 200)}`
    );
  }

  return {
    rootContent: rootMatch[1].trim(),
    timelineScript: timelineMatch[1].trim(),
  };
}
...
function initProject(projectId) {
  ...
  try {
    ...
    const details = (err.stdout?.toString() || "") + (err.stderr?.toString() || "");
    throw new Error(`Échec de l'initialisation du projet HyperFrames :\n${details}`);
  }
  return projectDir;
}
...
function lintProject(projectDir) {
  try {
    ...
    const details = (err.stdout?.toString() || "") + (err.stderr?.toString() || "");
    throw new Error(`Échec du lint HyperFrames :\n${details}`);
  }
}
...
function renderProject(projectDir, outputPath, variables) {
  ...
  try {
    ...
    const details = (err.stdout?.toString() || "") + (err.stderr?.toString() || "");
    throw new Error(`Échec du rendu HyperFrames :\n${details}`);
  }
  return outputPath;
}
